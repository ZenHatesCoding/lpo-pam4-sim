...
- **三组自由度分工（v5 关键改动）**：
  - **gain 维**：**不进代理**。每步用发端指标（MZM 输入 RMS，发端可测、不需 BER）解析调到
    `TARGET_DRIVE_RMS = 0.14 V`。因 `drive_rms ∝ gain` 且 k 随 IL 变，解析出的 gain 倍率自动从
    强信号环境的 ~0.8 调到弱信号环境的 ~1.3。
...
3. **target_rms 必须扫描标定**：不是拍脑袋。`scratch/scan_env_optimal.py` 全环境扫描，
   0.14 V 是几何均值最优。换器件后重跑扫描。
...
# 3) 全环境扫描标定 target_rms（换器件/换用例时重跑）
python scratch/scan_env_optimal.py --jobs 12

# 4) 在线调优（15 场景，gain 发端 RMS 物理驱动）
...
- 扫描标定：`result/env_optimal_scan.csv`（15 环境 × gain×CTLE 网格，target_rms 设计依据）
...
2. **target_rms=0.14 是离线标定常数**：换器件后需重跑 `scratch/scan_env_optimal.py`。
