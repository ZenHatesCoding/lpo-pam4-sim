...
本版在线调优演示从次优起点（gain ×0.80，接近标称）出发，梯度把 gain 推到各用例最优倍率附近（见 6.1）。</p>
...
<div class="card">
  ...
  <ol style="margin-bottom:0">
    ...
    <li>gain 是第 7 个搜索维：经 drive_rms 进入 A/B 输入，在 ±0.15 dex 信任域内参与梯度；本版在线调优从次优起点（gain ×0.80、接近标称）出发，梯度把 gain 推到各环境 BER 最优倍率。</li>
    <li>改善主要来自 gain 维（第 7 维）和形状（FFE/gDC/gDC2）两部分：从 ~1e-5 的次优点出发，梯度把 BER 压到检测限附近（强信号）或 1e-6 量级（极端插损）。Model B 全程未触发拦截，在本实验里是保险，不是必需。</li>
  </ol>
</div>
