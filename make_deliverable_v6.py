...
<figure>
  ...
  <svg class="d-wide" viewBox="0 0 1080 320" role="img" aria-label="优化空间参数化示意">
    ...
    <text class="ts" x="466" y="128" text-anchor="middle">4 个旁瓣 = 自由变量，|t| ≤ 0.3，Σ|旁瓣| ≤ 0.8 ⇒ 主抽头 t₄ = 1 − Σ|旁瓣| ≥ 0.2</text>
...
<figure>
  ...
  <svg class="d-wide" viewBox="0 0 1080 260" role="img" aria-label="双代理模型数据通路">
    ...
    <text class="tw2" x="89" y="62" text-anchor="middle">5-tap FFE + CTLE</text>
    ...
    <text class="tw2" x="189" y="192" text-anchor="middle">直接配置：5 抽头 + gDC + gDC2 + driver_gain</text>
  ...
  <svg class="d-narrow" viewBox="0 0 360 560" role="img" aria-label="双代理模型数据通路（竖向）">
    ...
    <text class="ts" x="24" y="62">6 维搜索点</text>
    ...
    <text class="tw2" x="24" y="348">5 抽头 + gDC + gDC2 + driver_gain</text>
...
<figure>
  ...
  <svg class="d-wide" viewBox="0 0 1080 500" role="img" aria-label="两阶段算法流程图">
    ...
    <text class="ts" x="48" y="170">基线 320 点 + 其余 14 环境各 60 点 + 每环境 1 个种子点 = 2001 点</text>
    ...
    <text class="t" x="588" y="154">对 Model A 求有限差分梯度（6 维）</text>
  ...
  <svg class="d-narrow" viewBox="0 0 360 940" role="img" aria-label="两阶段算法流程图（竖向）">
    ...
    <text class="ts" x="32" y="144">基线 320 + 14 环境各 60 + 种子 = 2001 点</text>
    ...
    <text class="t" x="32" y="526">Model A 有限差分梯度（6 维）</text>
...
<div class="card">
  <ol style="margin-bottom:0">
    ...
    <li><strong>梯度</strong>：对 Model A 做 6 维有限差分，<span class="mono">eps = 0.01</span>，共 7 次评估（每次 = 1 探针 + 1 A 前向）。</li>
...
<div class="card" style="border-left:4px solid #c0392b">
  ...
  <table class="wide">
    ...
    <tr><td>2001 行（只用基线 Base_IL10x10）</td><td class="n win">0 步劣化</td><td class="n win">0</td><td class="n win">0</td><td class="n win">0</td><td>红线有效，平均 ×3.13</td></tr>
    <tr><td>2001 行（只用基线）</td><td class="n" style="color:#c0392b">60 步劣化</td><td class="n" style="color:#c0392b">60</td><td class="n" style="color:#c0392b">54</td><td class="n" style="color:#c0392b">52</td><td>任何裕度都挡不住，平均 ×1.66</td></tr>
  </table>
  </div>
...
6 维空间下 2001 行基线数据不足以让代理在信任域内保持方向可信。见 docs/10。</p>
<div class="tw">
...
<p>全部 2001 行参与训练（6 维 LHS 邻域采样），Model A 输入 = 7-tap Tx FIR 探针 + drive_rms（8 维波形域）-> log10(BER) 条件均值。
...
<div class="card">
  ...
  <ol style="margin-bottom:0">
    ...
    <li><strong>6 维空间可以被有效利用</strong>：2001 行训练配置下，15/15 用例正向、
...
<div class="card" style="border-left:4px solid #b26a00">
  ...
  <p style="margin-bottom:0"><strong>严格泛化（只用 2001 行基线训练）在 6 维空间下数据不足</strong>：
...
<div class="card" style="border-left:4px solid #0f8a4a">
  ...
  <ol style="margin-bottom:0">
    <li><strong>加密基线采样</strong>：321 → 640～960 点（6 维下每轴覆盖仍偏少）。后续可调 CTLE 中心差分步长或用解析链式雅可比，
      算法无需改动 —— 最直接的补救。</li>
...
<div class="card">
  ...
  <pre><code># 1) 数据集（2001 点；6 维 LHS；多进程；与串行逐位一致）
...
python -c "from train_surrogates import train_v6; import glob; \
  train_v6(sorted(glob.glob('dataset/ddps_v6_dataset_*.csv'))[-1], 'models/ddps_v6')"
python run_ddps_v6_control.py --dataset dataset/ddps_v6_dataset_&lt;ts&gt;.csv \
    --base-env Base_IL10x10 --model-dir models/ddps_v6_control \
    --test-out result/ddps_v6_control --num-symbols 262144 --sim-seeds 42,43,44

# 3) 在线调优（15 环境）
python test_generalization.py --model-dir models/ddps_v6 --out-dir result/ddps_v6_&lt;ts&gt; \
    --num-symbols 262144 --sim-seeds 42,43,44 --n-steps 15 --cloud-n 8
python test_generalization.py --model-dir models/ddps_v6_control \
    --out-dir result/ddps_v6_control_ffe_only --freeze-extra \
    --num-symbols 262144 --sim-seeds 42,43,44 --n-steps 15

# 4) 报告与跨实验汇总
python report_ddps_v6.py --test-dir result/ddps_v6_&lt;ts&gt; --model-dir models/ddps_v6 \
    --deep-symbols 524288 --summary "result/ddps_v6_control:只用基线" \
    "v6:A探针+B参数" --summary-out result/SUMMARY.md</code></pre>
</div>
...
<table>
  ...
  <tr><td>数据集</td><td class="mono">dataset/ddps_v6_dataset_&lt;ts&gt;.csv</td><td>2001 行 × 44 列（6 维坐标 + 真实 BER + 驱动 RMS + FIR 形状）</td></tr>
  <tr><td>核心模型</td><td class="mono">models/ddps_v6_control/</td><td>A=探针 8 维 / B=参数 7 维 + meta.json</td></tr>
    <tr><td>核心结果</td><td class="mono">result/ddps_v6_&lt;ts&gt;/</td><td>case_summary.csv/json、trace_&lt;用例&gt;.csv、run_config.json、report/</td></tr>
