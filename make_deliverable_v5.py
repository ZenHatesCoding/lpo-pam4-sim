...
<header class="top">
  <div class="wrap">
    <div class="eyebrow">LPO 112G PAM4 仿真平台 · 发送端 6 维梯度下降 + gain 发端 RMS 物理驱动</div>
    <h1>DDPS：只用基线训练、跨场景泛化的发送端联合寻优 — 交付说明</h1>
    <div class="scope">
      本文说明方案的适用范围、物理链路与优化对象、两个代理模型各自在训练什么、在线调优算法与全部超参的标定依据、15 个用例的实测结果与逐用例参数对照。
      参数取自 <code>config.xlsx</code>、模型 <code>meta.json</code>、结果 <code>case_summary / trace</code> 与源码常量；图表为内嵌 SVG 与 PNG，单文件可离线打开。
    </div>
    <div class="chips">
      <span class="chip">代理搜索空间 <b>6</b> 维（4 个 5-tap FFE 旁瓣 + CTLE×2）；gain 由发端 RMS 物理目标驱动</span>
...
<div class="card">
  ...
  <ul style="margin-bottom:0">
    <li>LPO 光模块内部不做重 DSP，发送端均衡由 Host ASIC 承担。可调量只有三类：<strong>5-tap T-spaced 发送端 FFE</strong>（4 个旁瓣自由变量 + 1 个派生主抽头）、<strong>发送端模拟 CTLE 的双级直流增益</strong>、<strong>Driver 的真实线性增益</strong>。其中 FFE+CTLE 共 6 维交给代理梯度下降；gain 维由发端 RMS 物理目标驱动，不进代理。</li>
    <li>信道条件：奈奎斯特电插损 Tx/Rx <strong>各自</strong> 10～20 dB，色散 0～28 ps/nm，差分群时延 0～5 ps，偏振角 0～45°，另含器件噪声应力（RIN / 消光比 / TIA 噪声）。</li>
    <li>约束：在线调优阶段<strong>不得使用真实收端误码做决策</strong>；每一步落地前须通过"预测不劣化"的安全性审查，真实 BER 只做旁路记账与事后核验。gain 维用发端指标（MZM 输入 RMS），不属于收端 BER。</li>
  </ul>
</div>
...
<div class="card">
  ...
  <ol style="margin-bottom:0">
    <li><strong>代理的搜索向量是 6 维</strong>：<code>x_shape = [4 个 FFE 旁瓣, gDC, gDC2]</code>；主抽头由 <code>1 − Σ|旁瓣|</code> 派生，不占自由度。<strong>gain 不在搜索向量里</strong>——它由发端 RMS 物理目标每步解析求解（见 §3）。</li>
    <li><strong>两个代理都在这 6 维上回归同一个标签</strong> <code>log10(BER_MLSE)</code>（只用基线环境的 2001 个真实样本训练）：Model A 拟合它的<strong>条件均值</strong>，给出梯度方向；Model B 拟合它的<strong>保守上包络</strong>，做安全否决。二者的输入、标签完全相同，差别只有一个：A 求准，B 求守。</li>
    <li><strong>FFE/CTLE 做投影梯度下降，gain 做发端 RMS 物理驱动</strong>：方向取 Model A 的解析梯度，步长按"组内归一化 × 该组箱宽"分配，候选点须通过 Model B 的百分比红线；gain 在 FFE/CTLE 候选定下后解析调到 per-case target_rms（每个用例单独扫描标定）；另有<strong>轨迹信任域</strong>限制总位移。</li>
  </ol>
</div>
...
<figure>
  ...
  <svg class="d-wide" viewBox="0 0 1080 260" role="img" aria-label="双代理模型数据通路">
    ...
    <text class="tw2" x="164" y="192" text-anchor="middle">同一输入 x（6 维）</text>
  ...
  <svg class="d-narrow" viewBox="0 0 360 560" role="img" aria-label="双代理模型数据通路（竖向）">
    ...
    <text class="tw2" x="24" y="348">同一输入 x（6 维）</text>
...
<div class="card">
  ...
  且 7 抽头绝对 FIR 对 6 维配置是<strong>多对一</strong>压缩。实测同一份数据上，二阶多项式基对波形特征的留出集
...
<figure>
  ...
  <svg class="d-wide" viewBox="0 0 1080 500" role="img" aria-label="两阶段算法流程图">
    ...
    <text class="ts" x="48" y="294">γ 中位距离启发式、α 由 5 折 CV 选；80/20 划分，seed 42（A/B 均 6 维）</text>
    ...
    <text class="t" x="588" y="154">对 Model A 求解析梯度（6 维）</text>
    <text class="ts" x="588" y="170">∂f/∂x_shape 闭式给出；单步 ≈2 ms，无仿真、无探针</text>
  ...
  <svg class="d-narrow" viewBox="0 0 360 940" role="img" aria-label="两阶段算法流程图（竖向）">
    ...
    <text class="t" x="32" y="526">Model A 解析梯度（6 维）</text>
...
<div class="card">
  ...
  <ul style="margin-bottom:0">
    <li><strong>采样器</strong>：<span class="mono">LatinHypercube(d = 6, seed = 42)</span>，核心段与外壳段各用同一 LHS 序列的对应行，逐行可复现。</li>
    <li><strong>每行字段</strong>：6 维搜索坐标 <span class="mono">x_0…x_5</span>、5-tap FFE、gDC/gDC2、driver_gain 与倍率、真实 <span class="mono">mlse_ber</span> / <span class="mono">log10_ber_mlse</span>、<span class="mono">ber_std_log10</span>、7-tap FIR（<strong>诊断列</strong>，不进入模型输入）。</li>
...
<div class="tw">
<table class="wide">
  <caption>逐用例：种子 → 收敛（6 个自由变量） <span class="sh">· 可左右滑动</span></caption>
...
def main():
    ...
    ap.add_argument('--baseline', default='result/ddps_v5_main', help='只用基线训练（核心实验）')
    ap.add_argument('--model-dir', default='models/ddps_v5')
    ap.add_argument('--dataset', default=None)
    ap.add_argument('--local-grad', default='result/ddps_v5_local_gradient.csv')
    ...
    ap.add_argument('--divergence', default='result/ddps_v5_divergence.csv')
    ap.add_argument('--per-case-rms', default='result/per_case_target_rms.json')
    ...
    headline = '\n'.join([
        ...
        f"<tr><td><strong>FFE/CTLE 6 维梯度下降 + gain 发端 RMS 物理驱动</strong>（5-tap FFE 旁瓣 4 + CTLE×2 走代理；driver gain 每步解析调到 per-case target_rms）</td>"
        f"<td>driver 增益倍率随场景自适应 {kpi['{{KPI_GAIN}}']}；逐个用例的完整参数对照见 §6.2</td></tr>",
        f"<tr><td>代理方向到底可不可信（6 轴中心差分实测，不参与训练）</td>"
        f"<td>方向命中率 <strong>{hit_a:.2f}</strong>，按 |实测斜率| 加权 <strong>{wh_a:.2f}</strong>，"
        f"量级相关 {corr_a:+.2f}</td></tr>",
        f"<tr><td>预测 vs 实测：方向是否一致</td>"
        f"<td>15 个场景 Δ预测与 Δ实测<strong>全部正相关</strong>"
        f"（{pos_corr}/{n_div}，相关中位 {corr_med:+.2f}）—— gain 维物理驱动后不再出现方向反转"
        f"（独立重仿真复核见 §9）</td></tr>",
        "<tr><td>在线决策是否使用真实收端误码</td><td><strong>不使用</strong>，仅旁路记录用于事后核验；gain 维用发端 RMS（不属收端 BER）</td></tr>",
    ])
