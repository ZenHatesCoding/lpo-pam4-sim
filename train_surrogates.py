...
# ============================================================================
# DDPS v5 训练接口：模型只建模 FFE+CTLE（6 维成形），gain 维由发端 RMS 物理目标驱动
#
# 根因（见 ddps_optimizer v5 注释与 scratch/diag_gain_sweep.py）：gain 维的最优方向
# 随环境反转（基线该降、高插损该升），信道盲的代理无法处理。v4 把 gain 交给代理导致
# 在恶劣环境反方向驱动、"预测降/实测升"。v5 把 gain 维移出代理，改为每步解析调到
# TARGET_DRIVE_RMS（发端指标，不需 BER）。
#
# 模型 A/B 输入 = x_shape = [4 FFE 旁瓣, gDC, gDC2]（6 维），与 v4 的 7 维相比只是
# 去掉了 u_gain 维。训练数据用全环境采样（FFE/CTLE 成形 + 多 gain），但**模型只学
# FFE/CTLE 方向**；gain 对 BER 的影响不进代理（由发端 RMS 物理目标处理）。
#
# 关键：训练时**每个样本的 BER 是在该样本 (FFE,CTLE,gain) 下的真实 BER**，但模型输入
# 只取 x_shape —— 同一个 (FFE,CTLE) 不同 gain 下 BER 不同，代理学的是"条件均值"。
# 这正是 v3 的做法（v3 模型也只看形状，gain 用发端 RMS 特征补充），v5 更彻底地分离：
# 代理只负责能泛化的成形方向，gain 完全交给物理。
# ============================================================================
V5_SHAPE_COLS = [f'x_{i}' for i in range(6)]   # x_0..x_5 = 4 FFE 旁瓣 + gDC + gDC2


def train_v5(dataset_csv, model_dir="models", label_col=None, verbose=True,
             test_size=0.2, seed=42, env_filter=None):
    """DDPS v5：训练只建模 FFE/CTLE 成形方向的核岭代理（6 维）。

    env_filter=None 时用全部环境样本（让代理见到不同 IL 下的成形-BER 关系，增强泛化）；
    也可只取基线（env_filter='Base_IL10x10'）做纯基线训练对比。
    """
    import pandas as pd
    import json as _json
    if label_col is None:
    ...
    xcols = sorted([c for c in df.columns if c.startswith('x_')],
                   key=lambda c: int(c.split('_')[1]))[:6]   # 只取前 6 维（FFE+CTLE）
    X = df[xcols].values.astype(float)
    y = df[label_col].values.astype(float)
    tr, te = _train_test_split_idx(len(df), test_size, seed)
    ...
    envs = df['env'].values if 'env' in df.columns else None
    meta = {
        'dataset_csv': dataset_csv, 'label_col': label_col, 'pipeline': 'ddps_v5',
        'n_rows': int(len(df)), 'n_train': int(len(tr)), 'n_test': int(len(te)),
        'env_filter': env_filter,
        'model_a_features': xcols, 'model_b_features': xcols,
        'model_a_dim': len(xcols), 'model_b_dim': len(xcols),
        'model_a_type': 'WhiteBoxKernelRidge(RBF, 6-dim FFE+CTLE only)',
        'model_b_type': 'WhiteBoxEnvelope(mean + c*residual-scale ridge)',
        'gamma': float(model_a.gamma), 'alpha': float(model_a.alpha),
        'cv_mse': float(cv), 'envelope_c': float(c_cal),
        'envelope_coverage_test': float(cov_cal),
        'local_spacing_sigma': float(rho),
        'gain_mode': 'physical_target_rms (not surrogate-driven)',
        'target_drive_rms': 0.14,
        'model_a': {
            'r2_test': _r2_score(y[te], pa), 'mse_test': _mse(y[te], pa),
            'spearman_test': _spearman(y[te], pa), 'pred_std_test': float(np.std(pa)),
        },
    ...
    if envs is not None:
        ...
        for tag, pred in [('model_a', pa), ('model_b', pb)]:
            ...
            for e in np.unique(env_te):
                ...
                if m.sum() >= 5:
                    by_env[str(e)] = {'n': int(m.sum()),
                                       'spearman_test': _spearman(y[te][m], pred[m])}
            meta[f'{tag}_by_env'] = by_env
    if verbose:
        print(f"Model A (x_shape -> mean logBER): R2={meta['model_a']['r2_test']:.3f} "
              f"Spearman={meta['model_a']['spearman_test']:.3f} "
              f"(gamma={model_a.gamma:.4f}, alpha={model_a.alpha:g}, CV-MSE={cv:.4f})")
        print(f"Model B (x_shape -> envelope):    R2={meta['model_b']['r2_test']:.3f} "
              f"Spearman={meta['model_b']['spearman_test']:.3f} "
              f"(c={c_cal:g}, coverage={meta['model_b']['coverage_test']:.2f})")
...
# ============================================================================
# DDPS v6 双代理：恢复 v2 的 A/B 分工（A=探针→BER，B=参数→BER）
#
...
# 梯度下降通过 A 的链式法则：扰动 6 维参数 -> 重算探针 -> 查 A -> 得方向。
# ============================================================================

V6_A_COLS = [f'tx_fir_{i}' for i in range(7)] + ['drive_rms']
V6_B_COLS = [f'x_{i}' for i in range(6)] + ['drive_rms']
...
def train_v6(dataset_csv, model_dir="models", label_col=None, verbose=True,
    ...
    X_A = df[V6_A_COLS].values.astype(float)
    X_B = df[V6_B_COLS].values.astype(float)
    y = df[label_col].values.astype(float)
    envs = df['env'].values if 'env' in df.columns else None
    ...
    meta = {
        ...
        'model_b': {
            ...
            'description': '6 维 x_shape + drive_rms -> log10 BER 保守上包络（风险控制）',
