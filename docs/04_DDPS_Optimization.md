...
> **已知限制（当前物理底座）**：在补回 Driver 显式增益、ENOB 量化、激光相位噪声后，链路噪声显著上升，Model A（7-tap FIR → BER）的测试 R² 已降至 ~0.17，其有限差分梯度方向不再可靠，Stage 2 下降会沿噪声梯度发散（真实 BER 从种子点 `~1e-3` 一路到 `~9e-2`）。这是代理在噪声主导区域失效的体现，需后续用更高保真发端特征、或让 Model B 主导下降来修复。详见 `result/ddps/generalization_summary_physical.md`。
