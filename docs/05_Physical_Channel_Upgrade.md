...
- **Driver 显式增益（与带限解耦）**
  链路顺序严格对齐 SJTU，并在 v3 加入 post-channel CTLE：

  `Tx 电插损 -> 1mV 前端热噪声 -> Tx 模拟 CTLE -> VGA 摆幅控制 -> Driver 真增益 -> Driver 带限`
  - `driver_gain`（线性，标定值 2.0 ≈ 6 dB，对应 SJTU `DriverParams.Vpp=2.0`）是**真实线性增益**；
    **v3 起它是可优化维度**（1.0～3.0），因为它直接决定 MZM 驱动幅度：增益越大，OMA 越大（对抗收端噪声），
    但越容易把 MZM 推入非线性区。实测 gain=3.0 时 BER 相对 gain=2.0 劣化约 20 倍，gain=1.0～1.5 则优于标定值。
  - VGA 位于 CTLE 之后、Driver 之前，把信号归一化到**固定 RMS**（`vga_out_rms`），且**不依赖 driver_gain**：
    这样 CTLE 只负责频谱整形（其直流增益被 VGA 吸收），驱动摆幅只由 driver_gain 决定，两个维度互不冗余。
    *(v2 时代 VGA 归一化里除了 driver_gain、后面又乘回去，二者完全抵消，导致该参数对信号与噪声都没有影响 —— v3 已修正。)*
  - 关键点：1 mV 前端热噪声被放在**增益之前**，因此会被 CTLE + VGA + Driver 增益一起放大——插损越高、补偿增益越大、前端噪声被放大越多。这纠正了此前"增益免费、噪声不放大"的错误。
