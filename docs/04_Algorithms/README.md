...
**简介**：完全离线的“数据驱动”路线。Stage 1 只产出“不错起点 + 双 Ridge 代理”（Model A 看发端物理 FIR 作寻优目标、Model B 看原始配置作安全约束，不追求穷尽地形）；Stage 2 在 Model A 上做带 Model B 安全约束 + 信任域的梯度下降，**不回传真实 BER**，通过优化发端指标等效优化收端 MLSE BER，并能下钻到 Stage 1 采样没见过的更优点。
