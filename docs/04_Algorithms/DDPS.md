...
### 为什么是 0.70，怎么抬？（特征消融，`feature_ablation.py`，300 配置）

| 特征 | 维度 | 测试 R² | 测试 Spearman |
| --- | --- | --- | --- |
| FIR 7-tap | 7 | `0.242` | `0.702` |
| FIR 15-tap | 15 | `0.253` | `0.689` |
| FIR 31-tap | 31 | `0.253` | `0.689` |
| **配置 (9-tap + CTLE)** | 10 | `0.716` | **`0.880`** |
| FIR7 + 配置 | 17 | `0.725` | `0.877` |
| FIR15 + 配置 | 25 | `0.727` | `0.878` |

**结论**：
1. **加 FIR 抽头没用**（7/15/31 几乎一样）：0.70 不是截断损失，是 7-tap FIR 的**信息上限**——它只编码了确定性
   等效信道，编码不进噪声与收端 FFE/MLSE 的交互。
2. **配置（FFE 抽头 + CTLE）本身排序更强（0.88）**：配置直接决定均衡，且 Ridge 多项式能很好地拟合配置→BER。
3. **FIR + 配置 ≈ 配置**：FIR 是配置的确定性压缩，叠在配置上不增加信息。
4. **关键提醒**：这是**仿真器里的结论**（配置→FIR 是精确映射）。真实系统里配置→BER 有未建模效应（非线性、温漂、
   老化），届时**实测 FIR 可能比"理想配置"更可靠**——这一点在仿真器里无法验证，需要真实链路数据才能定论。
