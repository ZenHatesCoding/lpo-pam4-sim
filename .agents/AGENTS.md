...
- **模型泛化性评估方法论 (Model Generalization Methodology)**：在评估代理模型的泛化能力时，严禁针对每个评估用例执行“重新采样、重新训练”。正确的验证逻辑必须基于控制变量法：(1) 在基线（单一或混合环境）上训练并固化模型；(2) 在每次仅改变单一物理变量（如插损、色散或DGD）的新信道环境中，直接复用该模型进行测试。通过此方法确立模型在物理参数漂移时的衰减边界，从而回答“单点模型能否直接泛化”、“是否必须引入多环境混合数据”等核心科学问题。
