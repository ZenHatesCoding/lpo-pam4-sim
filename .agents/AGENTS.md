...
- **物理代价忠实记录 (Faithful Physical Cost Recording)**：在编写或评估任何在线硬件寻优算法时，必须恪守“所测即所发生”的原则。只要算法将一组系数下发至评估函数进行真实 BER 测试，无论该系数引发的误码多差、算法内部在逻辑上如何“丢弃/拒绝”该次优解，其付出的物理代价必须被无条件、全量地记录在历史收敛曲线和 Max BER 安全评估指标中。绝不允许在最终统计中隐藏任何破坏性试探。
