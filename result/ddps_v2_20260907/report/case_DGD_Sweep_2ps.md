...
- 物理环境：Tx/Rx 插损 `10 dB`，CD `0 ps/nm`，DGD `2 ps`（偏振 45°）
...
| 项 | 数值 |
| --- | --- |
| 种子 BER_MLSE | `4.42e-04` |
| Stage-2 最优 BER_MLSE | `3.35e-04` (step 4) |
| 收敛终点(final) BER_MLSE | `3.88e-04` |
| Δlog10 (seed→best) | `-0.12` |
| 实际步数 | 12 |
| 全程最差步 max | `3.88e-04`（真实 BER 全记录，无一步劣于种子） |
| 轨迹一致性 Spearman(ModelA, real) | 0.418 |
| 种子邻域云校验 n=16 Spearman(A,real) | 0.882 |
| 深水复核 seed→best (262144 sym) | `2.72e-04` → `1.67e-04` (Δ `-0.21`) |

## 图件（优化前后对照）

### 收敛轨迹 + Tx FFE 抽头 + Tx CTLE 响应 + Tx FIR 探针

![case DGD_Sweep_2ps part A](ddps_v2_case_DGD_Sweep_2ps_a.png)

### 眼图 / 频谱 / 均衡电平（seed vs best）

![case DGD_Sweep_2ps part B](ddps_v2_case_DGD_Sweep_2ps_b.png)
...
- 完整逐级 trace：[`../trace_DGD_Sweep_2ps.csv`](../trace_DGD_Sweep_2ps.csv)（csv，含每步 x/taps/gDC/gDC2/Model A/B 预测/真实 BER/梯度范数）

| step | real BER_MLSE | Model A pred | Model B pred | gDC (dB) | gDC2 (dB) |
| --- | --- | --- | --- | --- | --- |
| 0 | `3.84e-04` | `4.39e-03` | `1.34e-03` | `-0.007` | `-0.003` |
...
| 5 | `3.43e-04` | `3.16e-03` | `2.61e-03` | `-0.032` | `-0.012` |
